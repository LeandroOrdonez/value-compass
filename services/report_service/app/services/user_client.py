...
class UserServiceClient:
    """Client for interacting with the User Service internal API (service-to-service)"""
    ...
    async def get_portfolios(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        response = await self.client.get(f"/internal/users/{user_id}/portfolio")
        response.raise_for_status()
        return response.json()
    
    async def get_portfolio(self, user_id: int, portfolio_id: int) -> Dict[str, Any]:
        """Get a specific portfolio"""
        response = await self.client.get(f"/internal/users/{user_id}/portfolio/{portfolio_id}")
        response.raise_for_status()
        return response.json()
    
    async def get_portfolio_holdings(self, user_id: int, portfolio_id: int) -> List[Dict[str, Any]]:
        """Get holdings for a portfolio"""
        response = await self.client.get(f"/internal/users/{user_id}/portfolio/{portfolio_id}/holding")
        response.raise_for_status()
        return response.json()
    
    async def get_stock_baskets(self, user_id: int) -> List[Dict[str, Any]]:
        """Get stock baskets for a user"""
        response = await self.client.get(f"/internal/users/{user_id}/customBaskets")
        response.raise_for_status()
        return response.json()
    
    async def get_basket(self, user_id: int, basket_id: int) -> Dict[str, Any]:
        """Get a specific stock basket"""
        response = await self.client.get(f"/internal/users/{user_id}/customBaskets/{basket_id}")
        response.raise_for_status()
        return response.json()
    
    async def get_basket_stocks(self, user_id: int, basket_id: int) -> List[Dict[str, Any]]:
        """Get stocks in a basket"""
        response = await self.client.get(f"/internal/users/{user_id}/customBaskets/{basket_id}/stocks")
        response.raise_for_status()
        return response.json()
